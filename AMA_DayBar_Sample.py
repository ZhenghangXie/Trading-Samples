...
def set_info(context):
    ...
    g.Window = 15 # AMA等窗口参数（方便起见，系数统一））
    
    # 交易的期货品种信息
    g.instruments = ['TA','PP','CU','AG','L','RB','I','JD','CF','J']
...
def before_market_open(context):
    ...
    for ins in g.instruments:
        ...
        if dom == '':
            ...
            #print('TradeLots:%s'%(g.TradeLots[dom]))
...
def Trade(context,RealFuture,IndexFuture):
    ...
    if g.Signal == 1 and context.portfolio.long_positions[RealFuture].total_amount == 0:
        ...
        Chandelier_Exit(context,RealFuture,IndexFuture)
...
# 吊灯止损模块
def Chandelier_Exit(context,RealFuture,IndexFuture):
